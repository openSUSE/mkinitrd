...
#%programs: /bin/bash umount test mount mkdir ln date sleep echo cat /bin/sed /sbin/insmod /sbin/modprobe kill /sbin/killall5 /sbin/halt /sbin/reboot /sbin/showconsole cp /sbin/pidof mv chmod rm true ls /lib/mkinitrd/bin/*
...
mount -t devtmpfs -o mode=0755,nr_inodes=0${tmpfs_options:+,$tmpfs_options} devtmpfs /dev
